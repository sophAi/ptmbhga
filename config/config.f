       program config
       implicit none
       integer type,A,cent2,naccept,d,rec_sw,recover_sw,recover_rec,qscA
...
23     rewind(38)
     ...
     &",3rd=",mvj3
         ...
         write(*,*) "@ Max MC steps for each recursion for 1st Run=",
     &STEPMAX1
         write(*,*) "@ Multi canonical max number for 2nd Run=",MCMAX2
         write(*,*) "@ Max MC steps for each recursion for 2nd Run=",
     &STEPMAX2
         write(*,*) "@ Multi canonical max number for 3rd Run=",MCMAX3
         write(*,*) "@ Max MC steps for each recursion for 3rd Run=",
     &STEPMAX3
         if (d.eq.1) then
...
51             write(*,*) "For Multi-canonical Monte Carlo,MC must>1"
         ...
         write(*,*) "Monte Carlo step number for 1st Run:(default=5000)"
                 read(*,*) STEPMAX1
                 MCMAX1=1
...
52             write(*,*) "2nd MC RUN:"
          ...
          write(*,*)"Multi-Canonical MC number for 2nd Run:(default=30)"
           ...
           write(*,*)"Monte Carlo step number for 2nd Run:(default=500)"
                 read(*,*) STEPMAX2
                 MCMAX2=1
...
53             write(*,*) "3rd MC RUN:"
         ...
         write(*,*) "Monte Carlo step number for 3rd Run:(default=5000)"
     ...
     &tempf
            write(*,*) "Please input the number of steps(default=5000)"
                   read(*,*) PRE_BH
                 endif 
               endif
               goto 59             
...
C            <<<START MANY-BODY>>>
     ...
     &.or.z.eq.100.or.z.eq.110)then
               ...
               if(type.eq.2)then 
                 ...
                 read(*,*) A
                 if(A.eq.0.or.A.eq.atom_num)then
                   mvb1=0.D0
                   mvb2=0.D0
                   mvb3=0.D0
                 endif
