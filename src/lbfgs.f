...
C  Changed the convergence criterion to RMS. DJW
      IF (GNORM/DSQRT(1.0D0*N) .LE. EPS) FINISH=.TRUE.
C
      IF(IPRINT(1).GE.0) CALL LB1(IPRINT,ITER,NFUN,
     *               GNORM,N,M,X,F,G,STP,FINISH)
      IF (FINISH) THEN
...
C     -------------------------------------------------------------
C
      INTEGER IPRINT(2),I,ITER,NFUN,LP,MP,N,M
...
C     CHECK THE INPUT PARAMETERS FOR ERRORS.
C
      IF ((BRACKT .AND. (STP .LE. DMIN1(STX,STY) .OR.
...
C
         ...
         Q = (GAMMA + (DX - DP)) + GAMMA
         R = P/Q
         IF (R .LT. 0.D0 .AND. GAMMA .NE. 0.D0) THEN
            STPC = STP + R*(STX - STP)
