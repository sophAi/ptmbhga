     ...
     &gbin,ndim_temp,gbin_temp,dim3_temp
...
20     return
       ...
       implicit none
       integer ndim,seed,i,j,k,num,p1,p2,p3,p4,DG_RECOVER,dim3,pdb_rec
     &,gbin,ndim_temp,dim3_temp,gbin_temp
       real*8 xold(ndim),xnew(ndim),rnd,pei,r
       real*8 angle1,angle2,angle3,fac
       character name2*4,name6*6
       parameter(pei=3.14159D0)
...
20     return
     ...
     &,gbin,ndim_temp,dim3_temp,gbin_temp,J1,J2,J3,J4,gmin(ndim),
     &gmin_temp,J1_min,J2_min,guide_all(gbin*gbin*gbin)
       ...
       if(DG_RECOVER.eq.1)then
         ...
         read(49,*) ndim_temp,dim3_temp,gbin_temp
         if(ndim_temp.ne.ndim.or.dim3_temp.ne.dim3.or.gbin_temp.ne.gbin)
     &then
           write(*,*) "Waring!!READING DATA ERROR"
           goto 22
         endif
         write(49,*) (guide_all(J1),J1=1,gbin)
         do J1=1,ndim
           gmin_temp=0
           do J2=1,gbin
             do J3=1,ndim
               if(guide_all(J2).gt.gmin_temp.and.gmin(J1).ne.J2) then
                 gmin_temp=guide_all(J2)
                 gmin(J1)=J2
               endif
             enddo
           enddo
         ...
         close(49)
         do J1=1,ndim
           xnew(J1)=float(gmin(J1)-1)*DG-dsqrt(RADIUS)
         enddo  
       endif
22     do J1=1,ndim
